...
def test_server(data_dir):
    _send_hello_message(_HELLO_MESSAGE)
    _receive_config_message()
    _send_snapshot_message(_SNAPSHOT)
    _read_snapshot_message_from_queue(_SNAPSHOT)
...
def _send_hello_message(hello_message):
    with socket.socket() as connection:
        time.sleep(0.1)
        connection.settimeout(2)
        connection.connect(_SERVER_ADDRESS)
